"""Test comic format."""

import shutil
from datetime import datetime, timezone
from hashlib import sha256
from pathlib import Path
from types import MappingProxyType

from ruamel.yaml import YAML, SafeRepresenter
from treestamps.tree.config import TreestampsConfig

from picopt import PROGRAM_NAME, cli
from tests import IMAGES_DIR, assert_size_close, get_test_dir

__all__ = ()
TMP_ROOT = get_test_dir()
FN = "test_jpg.jpg"
SRC_JPG = IMAGES_DIR / FN
TMP_FN = str(TMP_ROOT / FN)
TIMESTAMPS_FN = f".{PROGRAM_NAME}_treestamps.yaml"
TIMESTAMPS_PATH = TMP_ROOT / TIMESTAMPS_FN
WAL_FN = f".{PROGRAM_NAME}_treestamps.wal.yaml"
WAL_PATH = TMP_ROOT / WAL_FN

FNS = MappingProxyType({FN: (97373, 87913)})

DEFAULT_CONFIG = {
    "bigger": False,
    "convert_jpeg_to_jxl": False,
    "convert_to": [],
    "convert_webp_to_jxl": False,
    "formats": ["GIF", "JPEG", "JXL", "PNG", "SVG", "WEBP"],
    "keep_metadata": True,
    "ignore": [],
    "near_lossless": False,
    "recurse": True,
    "symlinks": True,
    "timestamps_ignore_archive_entry_mtimes": False,
    # The tree has no .picopt.yaml files: the empty-digest fingerprint.
    "_dir_config_fingerprint": sha256(b"").hexdigest(),
}

# Written only by picopt 6.7.x and earlier, via treestamps 4.x. Fixtures
# keep emitting it to prove those files still load.
LEGACY_TREESTAMPS_CONFIG = {"ignore": frozenset(), "symlinks": True}


class TestTimestamps:
    """Test containers dir."""

    @staticmethod
    def _assert_sizes(index: int, root: Path = TMP_ROOT) -> None:
        """Assert sizes."""
        for name, sizes in FNS.items():
            path = root / name
            assert_size_close(path.stat().st_size, sizes[index])

    def setup_method(self: "TestTimestamps") -> None:
        """Set up method."""
        shutil.rmtree(TMP_ROOT, ignore_errors=True)
        TMP_ROOT.mkdir(exist_ok=True)
        shutil.copy(SRC_JPG, TMP_ROOT)
        self._assert_sizes(0)

    def teardown_method(self: "TestTimestamps") -> None:
        """Tear down method."""
        assert TIMESTAMPS_PATH.exists()
        assert not WAL_PATH.exists()
        shutil.rmtree(TMP_ROOT, ignore_errors=True)

    @staticmethod
    def _write_timestamp(
        path: Path | str,
        ts: float | None = None,
        config: dict | None = None,
    ) -> None:
        """Write timestamp."""
        if ts is None:
            ts = datetime.now(tz=timezone.utc).timestamp()
        if config is None:
            config = DEFAULT_CONFIG
        ts_config = {**LEGACY_TREESTAMPS_CONFIG}
        config = TreestampsConfig.normalize_config(config)
        ts_config = TreestampsConfig.normalize_config(ts_config)
        yaml = {"config": config, "treestamps_config": ts_config, str(path): ts}

        yaml_obj = YAML()
        yaml_obj.representer.add_representer(frozenset, SafeRepresenter.represent_set)
        yaml_obj.representer.add_representer(
            MappingProxyType, SafeRepresenter.represent_dict
        )
        yaml_obj.dump(yaml, TIMESTAMPS_PATH)
        assert TIMESTAMPS_PATH.exists()
        assert not WAL_PATH.exists()

    def test_no_timestamp(self: "TestTimestamps") -> None:
        """Test no timestamp."""
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(1)

    def test_timestamp(self: "TestTimestamps") -> None:
        """Test timestamp."""
        self._write_timestamp(TMP_FN)
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(0)

    def test_different_config(self: "TestTimestamps") -> None:
        """Test different config."""
        self._write_timestamp(FN)
        args = (PROGRAM_NAME, "-brtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(1)

    def test_config_missing_added_keys_honored(self: "TestTimestamps") -> None:
        """A stamp file predating a recorded key is still valid at its default."""
        # Exactly the shape picopt 6.6.x wrote, before the JXL options
        # joined the recorded key set.
        config = {
            key: value
            for key, value in DEFAULT_CONFIG.items()
            if key not in ("convert_jpeg_to_jxl", "convert_webp_to_jxl")
        }
        self._write_timestamp(TMP_FN, config=config)
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(0)

    def test_config_missing_key_with_other_value_invalidates(
        self: "TestTimestamps",
    ) -> None:
        """A missing key whose current value is not the default still invalidates."""
        config = {
            key: value for key, value in DEFAULT_CONFIG.items() if key != "bigger"
        }
        self._write_timestamp(FN, config=config)
        args = (PROGRAM_NAME, "-brtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(1)

    def test_config_missing_fingerprint_honored(self: "TestTimestamps") -> None:
        """A stamp file predating the fingerprint key is valid without dir configs."""
        config = {
            key: value
            for key, value in DEFAULT_CONFIG.items()
            if key != "_dir_config_fingerprint"
        }
        self._write_timestamp(TMP_FN, config=config)
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(0)

    def test_config_missing_fingerprint_invalidated_by_dir_config(
        self: "TestTimestamps",
    ) -> None:
        """The same old file is invalid once a sub-directory config exists."""
        config = {
            key: value
            for key, value in DEFAULT_CONFIG.items()
            if key != "_dir_config_fingerprint"
        }
        self._write_timestamp(FN, config=config)
        child = TMP_ROOT / "child"
        child.mkdir(exist_ok=True)
        (child / ".picopt.yaml").write_text("picopt:\n  bigger: true\n")
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(1)

    def test_generated_header_written(self: "TestTimestamps") -> None:
        """Stamp files explain themselves and omit the retired block."""
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        text = TIMESTAMPS_PATH.read_text()
        assert text.startswith("# @generated by treestamps ")
        assert PROGRAM_NAME in text.splitlines()[0]
        assert "do not edit" in text
        assert "Safe to delete" in text
        assert "treestamps_config" not in text

    def test_timestamp_dir(self: "TestTimestamps") -> None:
        """Test timestamp dir."""
        self._write_timestamp(TMP_ROOT)
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(0)

    def _setup_child_dir(self: "TestTimestamps") -> Path:
        """Set up child dir."""
        tmp_child_dir = TMP_ROOT / "child"
        tmp_child_dir.mkdir(exist_ok=True)
        shutil.move(TMP_ROOT / FN, tmp_child_dir)
        self._assert_sizes(0, tmp_child_dir)
        return tmp_child_dir

    def test_timestamp_children(self: "TestTimestamps") -> None:
        """Test timestamp children."""
        tmp_child_dir = self._setup_child_dir()
        self._write_timestamp(tmp_child_dir)
        args = (PROGRAM_NAME, "-rtvvvx SVG", str(TMP_ROOT))
        cli.main(args)
        self._assert_sizes(0, tmp_child_dir)

    def test_timestamp_parents(self: "TestTimestamps") -> None:
        """Test timestamp parents."""
        tmp_child_dir = self._setup_child_dir()

        self._write_timestamp(TMP_ROOT)
        args = (PROGRAM_NAME, "-rtvvvx SVG", str(tmp_child_dir))
        cli.main(args)
        self._assert_sizes(0, tmp_child_dir)
        assert (tmp_child_dir / TIMESTAMPS_FN).exists()
        assert not (tmp_child_dir / WAL_FN).exists()

    def test_journal_cleanup(self: "TestTimestamps") -> None:
        """Test journal cleanup."""
        args = (PROGRAM_NAME, "-rtvvvx SVG", TMP_FN)
        cli.main(args)
        assert not WAL_PATH.exists()

    @staticmethod
    def _write_wal(
        path: str, ts: float | None = None, config: dict | None = None
    ) -> None:
        """Write wal."""
        if ts is None:
            ts = datetime.now(tz=timezone.utc).timestamp()
        if config is None:
            config = DEFAULT_CONFIG
        ts_config = {**LEGACY_TREESTAMPS_CONFIG}
        config = TreestampsConfig.normalize_config(config)
        ts_config = TreestampsConfig.normalize_config(ts_config)
        yaml = {
            "config": config,
            "treestamps_config": ts_config,
            "wal": [{str(path): ts}],
        }
        yaml_obj = YAML()
        yaml_obj.representer.add_representer(frozenset, SafeRepresenter.represent_set)
        yaml_obj.representer.add_representer(
            MappingProxyType, SafeRepresenter.represent_dict
        )
        yaml_obj.dump(yaml, WAL_PATH)
        print(WAL_PATH.read_text())

    def test_timestamp_read_journal(self: "TestTimestamps") -> None:
        """Test timestamp read journal."""
        self._write_wal(TMP_FN)
        args = (PROGRAM_NAME, "-rtvvvx", "SVG", TMP_FN)
        cli.main(args)
        self._assert_sizes(0)
